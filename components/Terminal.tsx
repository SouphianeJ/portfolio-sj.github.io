...
export default function Terminal(){
  ...
  React.useEffect(()=>{
    let char = 0; let timer: any;
    function tick(){
      const line = LINES[i];
      setText(line.slice(0, char++));
      if(char > line.length){
        clearInterval(timer);
        setTimeout(()=>{ setI((p)=> (p+1)%LINES.length); }, 800);
      }
    }
  ...
  return (
    <div aria-live="polite" className="fixed right-4 bottom-4 w-[420px] max-w-[calc(100%-1rem)] text-green-400 brutal-card bg-black/90 text-sm">
